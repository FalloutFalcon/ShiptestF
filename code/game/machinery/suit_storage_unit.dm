...
/obj/machinery/suit_storage_unit/interact(mob/living/user)
	...
	if (locked)
		choices["unlock"] = icon('icons/hud/radial/menu.dmi', "radial_unlock")
	else if (state_open)
		choices["close"] = icon('icons/hud/radial/menu.dmi', "radial_close")
		...
		choices["open"] = icon('icons/hud/radial/menu.dmi', "radial_open")
		choices["disinfect"] = icon('icons/hud/radial/menu.dmi', "radial_disinfect")
		choices["lock"] = icon('icons/hud/radial/menu.dmi', "radial_lock")
...
/obj/machinery/suit_storage_unit/default_deconstruction_screwdriver(mob/user, icon_state_open, icon_state_closed, obj/item/I)
	if(!(flags_1 & NODECONSTRUCT_1) && (QUALITY_SCREW_DRIVING in I.tool_qualities) && uv)
...
/obj/machinery/suit_storage_unit/default_pry_open(obj/item/I)//needs to check if the storage is locked.
	. = !(state_open || panel_open || is_operational || locked || (flags_1 & NODECONSTRUCT_1)) && (QUALITY_PRYING in I.tool_qualities)
	if(.)
