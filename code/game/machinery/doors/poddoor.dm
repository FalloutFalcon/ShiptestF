...
/obj/machinery/door/poddoor/attackby(obj/item/W, mob/user, params)
	...
	if(resistance_flags & INDESTRUCTIBLE)
		return FALSE
	if((resistance_flags & INDESTRUCTIBLE) && (QUALITY_SCREW_DRIVING in W.tool_qualities)) // This makes it so ERT members cannot cheese by opening their blast doors.
	...
	if(QUALITY_DECONSTRUCT in W.tool_qualities)
		if(!(machine_stat & NOPOWER))
			do_sparks(5, TRUE, src)
