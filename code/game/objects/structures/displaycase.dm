...
/obj/structure/displaycase/attackby(obj/item/W, mob/user, params)
	if(W.GetID() && !broken && openable)
	...
	else if((QUALITY_WELDING in W.tool_qualities) && user.a_intent == INTENT_HELP && !broken)
		...
		return
	else if(!alert && (QUALITY_PRYING in W.tool_qualities) && openable) //Only applies to the lab cage and player made display cases
		if(broken)
			if(showpiece)
...
/obj/structure/displaycase_chassis/attackby(obj/item/I, mob/user, params)
	if(QUALITY_BOLT_TURNING in I.tool_qualities) //The player can only deconstruct the wooden frame
