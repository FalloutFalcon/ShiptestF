...
/obj/structure/table/attackby(obj/item/I, mob/user, params)
	...
	if(!(flags_1 & NODECONSTRUCT_1) && user.a_intent != INTENT_HELP)
		if(((QUALITY_SCREW_DRIVING in I.tool_qualities)) && deconstruction_ready)
			to_chat(user, span_notice("You start disassembling [src]..."))
			if(I.use_tool(src, user, 20, volume=50))
				deconstruct(TRUE)
			return

		if((QUALITY_BOLT_TURNING in I.tool_qualities) && deconstruction_ready)
...
/obj/structure/rack/attackby(obj/item/W, mob/user, params)
	var/list/modifiers = params2list(params)
	if ((QUALITY_BOLT_TURNING in W.tool_qualities) && !(flags_1&NODECONSTRUCT_1) && user.a_intent != INTENT_HELP)
...
/obj/structure/table/wood/reinforced/attackby(obj/item/W, mob/user, params)
	if((QUALITY_PRYING in W.tool_qualities) && user.a_intent != INTENT_HELP)
		if(!W.tool_start_check(user, amount=0))
			return
