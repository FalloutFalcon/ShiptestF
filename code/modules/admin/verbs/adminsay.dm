ADMIN_VERB(cmd_admin_say, R_NONE, "ASay", "Send a message to other admins", ADMIN_CATEGORY_MAIN, message as text)
	...
	var/custom_asay_color = (CONFIG_GET(flag/allow_admin_asaycolor) && user.prefs.asaycolor) ? "<font color=[user.prefs.asaycolor]>" : "<font color='#FF4500'>"
	message = "[span_adminsay("<span class='prefix'>ADMIN:")] <EM>[key_name_admin(user)]</EM> [ADMIN_FLW(user.mob)]: [custom_asay_color]<span class='message linkify'>[message]</span></span>[custom_asay_color ? "</font>":null]"
	to_chat(GLOB.admins,
