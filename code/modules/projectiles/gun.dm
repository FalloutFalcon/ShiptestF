...
/obj/item/gun/proc/try_sawoff(mob/user, obj/item/saw)
	if(!saw.get_sharpness() || !is_type_in_typecache(saw, GLOB.gun_saw_types) && !(QUALITY_SAWING in saw.tool_qualities)) //needs to be sharp. Otherwise turned off eswords can cut this.
		return
	if(sawn_off)
