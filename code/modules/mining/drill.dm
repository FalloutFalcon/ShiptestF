...
/obj/machinery/drill/attackby(obj/item/tool, mob/living/user, params)
	...
	if(machine_stat & BROKEN)
		if(istype(tool,/obj/item/stack/sheet/plasteel))
			...
			if(plating.use(10,FALSE,TRUE))
				...
				return
		if(metal_attached == METAL_SECURED && (QUALITY_WELDING in tool.tool_qualities))
			if(tool.use_tool(src, user, 30, volume=50))
				...
				update_icon_state()
				return
	if(QUALITY_BOLT_TURNING in tool.tool_qualities)
	...
	if(panel_open) //All malfunction repair and maintenance actions are handled under here
		...
		if(is_type_in_list(tool,needed_parts))
			for(var/obj/item/stock_parts/part in component_parts)
				...
				if(new_part.part_behaviour == part.part_behaviour)
					...
					break
			return
		if((QUALITY_PULSING in tool.tool_qualities) && malfunction == MALF_CALIBRATE)
			...
			if(tool.use_tool(src, user, 100, volume=50))
				...
				obj_integrity = max_integrity
				return
		if((QUALITY_WELDING in tool.tool_qualities) && malfunction == MALF_STRUCTURAL)
		...
		if(istype(tool, /obj/item/stock_parts/cell))
			...
			return
		if(QUALITY_PRYING in tool.tool_qualities)
