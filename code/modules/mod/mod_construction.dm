...
/obj/item/mod/construction/shell/attackby(obj/item/part, mob/user, params)
	...
	switch(step)
		if(START_STEP)
			...
			step = CORE_STEP
		if(CORE_STEP)
			if(QUALITY_SCREW_DRIVING in part.tool_qualities) //Construct
				if(part.use_tool(src, user, 0, volume=30))
					to_chat(user, span_notice("You secure the core."))
				step = SCREWED_CORE_STEP
			else if(QUALITY_PRYING in part.tool_qualities) //Deconstruct
				if(part.use_tool(src, user, 0, volume=30))
		...
		if(SCREWED_CORE_STEP)
			if(istype(part, /obj/item/mod/construction/helmet)) //Construct
				...
				helmet = part
				step = HELMET_STEP
			else if(QUALITY_SCREW_DRIVING in part.tool_qualities) //Deconstruct
				if(part.use_tool(src, user, 0, volume=30))
		...
		if(GAUNTLETS_STEP)
			if(istype(part, /obj/item/mod/construction/boots)) //Construct
				...
				if(part.use_tool(src, user, 0, volume=30))
					...
					step = CHESTPLATE_STEP
		if(BOOTS_STEP)
			if(QUALITY_BOLT_TURNING in part.tool_qualities) //Construct
				if(part.use_tool(src, user, 0, volume=30))
		...
		if(WRENCHED_ASSEMBLY_STEP)
			...
			else if(QUALITY_BOLT_TURNING in part.tool_qualities) //Deconstruct
				if(part.use_tool(src, user, 0, volume=30))
		...
		if(SCREWED_ASSEMBLY_STEP)
			...
			else if(QUALITY_SCREW_DRIVING in part.tool_qualities) //Construct
				if(part.use_tool(src, user, 0, volume=30))
					to_chat(user, span_notice("You unsecure the assembly."))
					step = SCREWED_ASSEMBLY_STEP
	update_icon_state()
