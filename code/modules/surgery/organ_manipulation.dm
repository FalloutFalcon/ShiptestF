...
/datum/surgery_step/manipulate_organs/preop(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	if(istype(tool, /obj/item/organ_storage))
		...
		if(!isorgan(manipulated_organ))
			to_chat(user, span_warning("You cannot put [manipulated_organ] into [target]'s [parse_zone(target_zone)]!"))
			return -1
	...
	if(isorgan(tool))
		...
		if(target_zone != manipulated_organ.zone || target.getorganslot(manipulated_organ.slot))
			to_chat(user, span_warning("There is no room for [manipulated_organ] in [target]'s [parse_zone(target_zone)]!"))
			return -1
		var/obj/item/organ/meatslab = tool
		if(!meatslab.useable)
			to_chat(user, span_warning("[manipulated_organ] seems to have been chewed on, you can't use this!"))
			return -1
	...
	if(istype(tool, /obj/item/mmi))//this whole thing is only used for robotic surgery in organ_mani_robotic.dm :*
		...
		if(!target_mmi.brainmob || !target_mmi.brainmob.client)
			to_chat(user, span_notice("[tool] has no life in it, this would be pointless!"))
			return -1
		...
		if(!organs.len)
			...
			if(manipulated_organ && user && target && user.Adjacent(target) && user.get_active_held_item() == tool)
				...
				if(!manipulated_organ)
					return -1
				display_results(user, target, span_notice("You begin to extract [manipulated_organ] from [target]'s [parse_zone(target_zone)]..."),
					span_notice("[user] begins to extract [manipulated_organ] from [target]'s [parse_zone(target_zone)]."),
...
/datum/surgery_step/manipulate_organs/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results)
	if(current_type == "posibrain")
		...
		if(manipulated_organ && manipulated_organ.owner == target)
			display_results(user, target, span_notice("You successfully extract [manipulated_organ] from [target]'s [parse_zone(target_zone)]."),
				span_notice("[user] successfully extracts [manipulated_organ] from [target]'s [parse_zone(target_zone)]!"),
